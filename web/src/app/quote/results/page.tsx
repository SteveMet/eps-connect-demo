...
import { ReasoningDrawer } from "@/components/results/reasoning-drawer";
import {
  FileText,
  Calculator,
  BarChart3,
...
export default function QuoteResultsPage() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        setModelName(sessionStorage.getItem("quoteModel"));
      } catch {
  ...
  return (
    <div className="px-6 py-6">
      ...
      {modelName && (
        <div className="mt-8 flex items-center justify-center gap-2 rounded-lg border border-border/50 bg-secondary/30 px-4 py-2.5">
      ...
      {/* Floating reasoning peek button */}
      <ReasoningDrawer />
    </div>
  );
}
